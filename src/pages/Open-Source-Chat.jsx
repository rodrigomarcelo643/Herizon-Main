...
import { BookText, Presentation, CircleDollarSign, MapPin, X, Send, Mic, MicOff } from "lucide-react";
...
export default function OpenSourceChat() {
  ...
  const generatePlan = async (prompt) => {
    ...
    try {
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: "system", 
              content: "Create a detailed business plan with sections for Executive Summary, Market Analysis, and Financial Projections." 
            },
      ...
      const needsMap = prompt.toLowerCase().includes("location") || prompt.toLowerCase().includes("map");
      if (needsMap) {
        setMapUrl(`https://www.google.com/maps/embed/v1/search?key=${GOOGLE_MAPS_KEY}&q=${encodeURIComponent(prompt)}`);
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          {plan && (
            <div className="bg-[#E1D7CD] text-gray-800 border border-white rounded-2xl p-4 mt-5 mb-10">
              ...
              {mapUrl && (
                <button onClick={() => setShowMapModal(true)} className="mt-4 flex items-center gap-2 text-sm bg-blue-500 text-white px-3 py-1 rounded-lg">
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="flex justify-center mt-4 flex-wrap gap-4">
            {quickActions.map((action, i) => (
              <button
                key={i}
                onClick={() => action.isMap ? (setShowMapModal(true), setMapUrl(`https://www.google.com/maps/embed/v1/search?key=${GOOGLE_MAPS_KEY}&q=${encodeURIComponent(action.text)}`)) : generatePlan(action.text)}
